...
check_database_user() {
	USER=$( echo "select host,user from mysql.user where user='$PF_DB_USER';" | mysql -u root --password=$PF_SETUP_PASS -h $PF_DB_HOST --skip-column-names --skip-ssl)
	if [[ -z "$USER" ]]
	then
		# Create user
		echo "Creating user..."
		echo "CREATE USER '$PF_DB_USER'@'%' IDENTIFIED BY '$PF_DB_PASS';" | mysql -u root --password=$PF_SETUP_PASS -h $PF_DB_HOST --skip-ssl
		if [[ $? -ne 0 ]]
		then
...
create_database() {
	...
	echo "CREATE DATABASE IF NOT EXISTS $PF_DB_NAME;" |  mysql -u root --password=$PF_SETUP_PASS -h $PF_DB_HOST --skip-ssl
	if [[ $? -ne 0 ]]
	then
	...
	echo "GRANT ALL PRIVILEGES ON \`$PF_DB_NAME\`.* TO '$PF_DB_USER'@'%' WITH GRANT OPTION; FLUSH PRIVILEGES;" | mysql -u root --password=$PF_SETUP_PASS -h $PF_DB_HOST --skip-ssl
	if [[ $? -ne 0 ]]
	then
...
create_tables() {
	mysql -u $PF_DB_USER --password=$PF_DB_PASS --skip-ssl -h $PF_DB_HOST $PF_DB_NAME <$IMAGE_HOME/create_tables.sql
	if [[ $? -ne 0 ]]
	then
...
check_database() {
	TABLES=$( echo "show tables;" | mysql -u $PF_DB_USER --password=$PF_DB_PASS -h $PF_DB_HOST --skip-column-names --skip-ssl $PF_DB_NAME)
	if [[ -z "$TABLES" ]]
	then
		...
		DATABASES=$( echo "show databases like '$PF_DB_NAME';" | mysql -u root --password=$PF_SETUP_PASS -h $PF_DB_HOST --skip-ssl --skip-column-names)
		if [[ $? -ne 0 ]]
		then
